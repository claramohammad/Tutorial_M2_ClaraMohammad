...
app.post('/inserePessoa', urlencodedParser, (req, res) => {
    ...
    res.setHeader('Access-Control-Allow-Origin', '*'); 
    var db = new sqlite3.Database(DBPATH); // Abre o banco
    sql = "INSERT INTO pessoa (id, nome, ocupacao) VALUES ('" + req.body.id + "', '" + req.body.nome + "', '" + req.body.ocupacao + "')";
    console.log(sql);
    db.run(sql, [],  err => {
        if (err) {
            throw err;
...
app.post('/atualizaPessoa', urlencodedParser, (req, res) => {
    res.statusCode = 200;
    res.setHeader('Access-Control-Allow-Origin', '*'); 
    sql = "UPDATE pessoa SET id='" + req.body.id + "', nome = '" + req.body.nome + "' , ocupacao='" + req.body.ocupacao + "'";
    console.log(sql);
    var db = new sqlite3.Database(DBPATH); // Abre o banco
    db.run(sql, [],  err => {
        if (err) {
